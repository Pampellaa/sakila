...
try:
    ...
    raise OperationalError(f'Connection error: {err}')
...
with cnx:
    ...
    #     print(error)
    try:
        cursor.execute(query_insert_user_tb,('Ola','o@a.pl','haslo123'))
    except DatabaseError as error:
        print(error)
    # try:
    #     cursor.execute(query_update_user_tb,('secret',1))
    # except DatabaseError as error:
    #     print(error)
    # try:
    ...
    try:
        ...
        rows = cursor.fetchall()   #w nawiasie ktora chcemy wybrac, nic to znaczy ze wszystkie
        for row in rows:
            print(row)
